...
char *perl_get_use_list(void)
{
  ...
  char *ret;
  const char *use_lib;
  const char *macirssi_lib;
  
	str = g_string_new(NULL);
  
	use_lib = settings_get_str("perl_use_lib");
  macirssi_lib = settings_get_str("macirssi_lib");
	g_string_sprintf(str, "use lib qw(%s/scripts "SCRIPTDIR" %s %s);",
                   get_irssi_dir(), use_lib, macirssi_lib);
