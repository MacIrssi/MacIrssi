...
{
	if (searchResult) {
		[textStorage addLayoutManager:[mainTextView layoutManager]];
		[[mainTextView layoutManager] setTextStorage:textStorage];
	...
	searchResult = [[NSTextStorage alloc] init];
	[searchResult addLayoutManager:[mainTextView layoutManager]];
	...
	while (TRUE) {
		...
		/* Check if threre are any lines left */
		if (lineRange.location == NSNotFound && searchRange.length == 0)
			break;
		...
		if (lineRange.location == NSNotFound) {
			...
			lineRange.length = lineRange.location - searchRange.location + 1;
			lineRange.location = searchRange.location;
		}

		line = [string substringWithRange:lineRange];
		r.length = [line length];
		NSRange tmp = [line rangeOfString:[sender stringValue] options:NSCaseInsensitiveSearch range:r];
		if (tmp.location != NSNotFound) {
			[searchResult appendAttributedString:[textStorage attributedSubstringFromRange:lineRange]];
		}
		
		searchRange.location += lineRange.length;
		searchRange.length -= lineRange.length;
		lineRange = [string rangeOfString:@"\n" options:NSCaseInsensitiveSearch range:searchRange];
	}

	[textStorage removeLayoutManager:[mainTextView layoutManager]];
	[[mainTextView layoutManager] setTextStorage:searchResult];
	[mainTextView setNeedsDisplay:TRUE];
}
...
- (void)finishLine
{
	BOOL scroll = [scroller floatValue] == 1.0;
	int i, loc = linebuf[0] == '\n';
...
- (void)saveScrollState
{
	...
	scrollState = [scroller floatValue] == 1.0;
}
